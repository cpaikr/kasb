...
try {
  ...
  if ([long]$ArchiveAsset.size -le 0 -or [long]$ArchiveAsset.size -gt 134217728) { throw "kasb: release archive metadata exceeds size limit" }
  if ([long]$ChecksumAssetMetadata.size -le 0 -or [long]$ChecksumAssetMetadata.size -gt 1048576) { throw "kasb: release checksum metadata exceeds size limit" }
